...
每次新会话启动时，框架会询问你使用哪种模式：

**监督模式** -- 每章逐步确认，人类全程参与。适合前期磨合（前5-10章）、重要剧情转折、需要精雕细琢的章节。每章有 3-4 次人类介入点（情绪板确认、初稿方向、正文审核、标题选定）。

**自主段落模式** -- 按段落（3-7章）批量生产，段落结束后统一审核。适合节奏稳定后的高速更新。需先完成段落规划（Phase 0），Claude 自动按顺序完成每章的 Phase 1-6，内置冷读测试和 OOC 检查作为自动质量门控。遇到 reader-pull 过低、事实冲突、重要战斗等情况时触发自适应中断，暂停等待人工决策。段落末章触发人类审核（Section 4.6）。

两种模式可随时切换：监督模式下完成段落规划后可进入自主模式；自主模式下任意章节完成后可切回监督模式。
...
每次会话启动时选择模式，随时可切换：
- **监督模式**：每章人工确认，适合精雕细琢和前期磨合
- **自主段落模式**：3-7章批量生产，段落结束后统一审核，适合高速更新
