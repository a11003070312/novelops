...
**节点1**〔轻〕节点名
一两句带过。这段不重要，交代作用即可，留给写手空间发挥。

**节点2**〔重〕节点名
用讲故事的语气详细描述这段：情绪是什么、最重要的画面或时刻是什么、读者应该感受到什么。不列步骤，不写"首先/然后"，写人在这个场景里的状态和感受。

**节点3**〔中〕节点名
说清楚方向和关键动作，其余留白。

（节点数量按章节实际内容决定，通常4-8个）

### 章末落点
明确写出最后一两句话应该落在哪里、让读者带走什么。子代理写完这里就停，不再追加。

### 角色
[每个出场角色：性格内核、说话风格、当前状态，几句话。紧张/战斗场景须注明台词风格]

### 硬约束
- 开章第一段自然带出时空坐标：用感知带出，不要直白说明
- [不能违反的关键事实]
- [章末必须落在的位置]

### 不要出现
- [角色在当前时间线尚未经历的事情——防止子代理编造经历]
- [容易被替换的地名/人名——写出正确的]
- [容易照搬的框架词/分析语言]
- [其他已知的幻觉风险]
...
### 前情
[最近2-3章发生了什么，用讲故事的语气。关键地点和人名写具体，防止子代理替换]
...
**节点权重说明：**
- 〔轻〕过渡、时间跳跃、氛围铺垫——一两句带过，留白给写手
- 〔中〕有明确功能但非核心高潮——给方向，不给细节
- 〔重〕核心情绪/核心逻辑/核心画面——详细描述，用叙事语气而非步骤清单

**"不要出现"使用规范：**
- 列出角色在当前章节时间线**尚未经历**的事（防止编造经历）
- 列出本章涉及的关键地名/人名的**正确写法**（防止替换）
- 列出容易被照搬进正文的**框架词**（防止分析语言污染）
- 每条简短，3-5条即可，不要变成全面禁忌清单

**组装要点：**
- 全文纯中文，不允许英文
- 角色 ID 替换为角色名，伏笔 ID 替换为描述
- **节点全部用叙事语言**——像导演给演员说戏，不像产品经理写需求文档
- 〔重〕节点写情绪和画面，不写执行步骤；〔轻〕节点一两句带过
- 硬约束只列事实性约束，不列文笔规则
- **情感节拍只写意图，不写画面**——禁止照搬大纲原文的具体动作/对话，用自己的语言重述目的和效果
- **绝不放入：** 禁用词清单、禁用句式清单、翻译腔规则、数值化指标、文笔样本
- **前情里关键地点和人名写具体**，不用模糊描述，防止子代理用已知名称填空
- **章节衔接检查（强制）：** 读取上一章摘要 events 最后一条，与本章 opening_notes 对比，如有跳跃在节点1补充过渡说明（不超过3句）。第1章或无前章摘要时跳过

**为什么这样分：** 实测验证（ch31），旧版"这一章写什么"像功能规格，子代理当清单执行，输出僵硬；新版关键节点按轻/中/重分配注意力，〔重〕节点用叙事语气描述，子代理进入故事状态而非实现状态，文笔明显提升。"不要出现"堵住了虚构经历和地名替换两类主要幻觉。
...
**强制前置：唯一性检查（监督模式和自主模式均须执行）**

选标题之前，必须运行以下命令获取所有已用标题列表：

```bash
python -c "
import glob, re, sys
titles = []
for f in glob.glob('chapters/arc-*/*.md'):
    with open(f, encoding='utf-8') as fh:
        for line in fh:
            m = re.match(r'^# \u7b2c[^\u7ae0]*\u7ae0[\s\u3000]*(.*)', line.strip())
            if m:
                titles.append(m.group(1).strip())
                break
sys.stdout.buffer.write('\n'.join(sorted(titles)).encode('utf-8') + b'\n')
"
```

候选标题不得与列表中任何已有标题完全相同（比对前去除两端标点，防止角括号变体漏过）。

**标题风格约束（减少碰撞概率）：**
- 优先用本章独有的具体意象、数字、动作（如"一寸一分""值命的习惯""窄天"）
- 避免使用叙事文本中高频出现的标签词（如"待查""待定""原因不明"——这些词在陆沉的册子里重复出现，极易碰撞）
- 避免过于通用的词（如"出发""决定""发现"）

**监督模式：** 人类确认正文内容后，根据最终定稿内容确定章节标题:

1. 运行唯一性检查命令
2. 回顾本章核心画面、情感要点、关键意象
3. 提出 3-4 个候选标题，每个附一句取意说明，所有候选均已通过唯一性检查
4. 候选标题的选取原则:
   - 从正文中提炼，不用大纲标题直接套用
   - 优先含蓄、有余味的（暗示 > 直白）
   - 可以是双关、意象、动作、物件
   - 避免剧透核心悬念
5. 人类选定后，更新章节文件的标题行和 chapter outline 的 title 字段

**自主模式：** 运行唯一性检查后，从已验证不重复的候选中选出最优标题，直接写入章节文件和 chapter outline 的 title 字段。在段落审核（Section 4.6）时人类可更改任意章节标题。
